...
>
    ...
    >
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4">
            ...
            {{-- CRM --}}
            <div x-data="{ crmOpen: {{ request()->routeIs('admin.crm.*') ? 'true' : 'false' }} }">
                <button
                    @click="crmOpen = !crmOpen"
                    class="group flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-sm font-medium transition-colors {{ request()->routeIs('admin.crm.*') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}"
                >
                    ...
                    <svg class="h-4 w-4 transition-transform" :class="crmOpen && 'rotate-180'" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                    </svg>
                </button>
                <div x-show="crmOpen" x-transition x-cloak class="mt-1 ml-8 space-y-0.5">
                    <a href="{{ route('admin.crm.contacts.index') }}"
                        @class([
                            'flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition-colors',
                            'bg-gray-700 text-white' => request()->routeIs('admin.crm.contacts.*'),
                            'text-gray-300 hover:bg-gray-800 hover:text-white' => !request()->routeIs('admin.crm.contacts.*'),
                        ])
                    >Contacts</a>
                    @role('super_admin')
                    <a href="{{ route('admin.crm.tags.index') }}"
                        @class([
                            'flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition-colors',
                            'bg-gray-700 text-white' => request()->routeIs('admin.crm.tags.*'),
                            'text-gray-300 hover:bg-gray-800 hover:text-white' => !request()->routeIs('admin.crm.tags.*'),
                        ])
                    >Tags</a>
                    <a href="{{ route('admin.crm.funnel-keys.index') }}"
                        @class([
                            'flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition-colors',
                            'bg-gray-700 text-white' => request()->routeIs('admin.crm.funnel-keys.*'),
                            'text-gray-300 hover:bg-gray-800 hover:text-white' => !request()->routeIs('admin.crm.funnel-keys.*'),
                        ])
                    >Funnel API Keys</a>
                    @endrole
                </div>
