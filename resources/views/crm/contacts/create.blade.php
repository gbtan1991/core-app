<x-layouts::admin.app title="Add Contact">
    ...
    <div class="mx-auto max-w-3xl">
        <div class="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('admin.crm.contacts.store') }}" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <select name="stage" required
                        ...
                        @foreach ($stages as $value => $label)
                            <option value="{{ $value }}" @selected(old('stage', 'lead') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="4"
                        class="mt-1.5 block w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-900 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20">{{ old('notes') }}</textarea>
                </div>
